...
import { AlternateKeyPoolEditor } from './AlternateKeyPoolEditor.tsx'
...
function draftAt(namespace: SettingsNamespaceView, path: readonly string[], schema: SettingsSchemaOperations): Record<string, unknown> {
...
export function ProviderEditor(props: ProviderEditorProps): ReactNode {
  const { schema, namespace, settingsPath, api, t } = props
  ...
  const node = useMemo(() => schema.nodeAtPath(root, settingsPath), [root, settingsPath, schema])
  ...
  const syncPoolNamespace = (updated: SettingsNamespaceView): void => {
    const updatedOriginal = schema.getPath(updated.user, settingsPath)
    const updatedPool = schema.getPath(updatedOriginal, ['apiKeyPool'])
    setCommittedOriginal(updatedOriginal)
    setExpectedRevision(updated.revision)
    setDraft(current => updatedPool === undefined
      ? schema.deletePath(current, ['apiKeyPool'])
      : schema.setPath(current, ['apiKeyPool'], updatedPool))
  }
      ...
      {layout === 'pi-ai' && fallback !== undefined && props.credentialOnly !== true
        ...
        ? (
          <AlternateKeyPoolEditor
            ...
            disabled={disabled}
            schema={schema}
            onNamespaceChange={syncPoolNamespace}
              />
        )
        : null}
