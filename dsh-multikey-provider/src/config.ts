...
import { MODALITIES, resolveRouteModels, SUPPORTED_THINKING_FORMATS, THINKING_LEVELS } from './provider/catalog.ts'
import type {
  PiAiCompatProfile,
  PiAiModality,
  PiAiModelOverride,
...
import { buildProvider, supportedProtocols } from './provider/provider.ts'
import { compileKeyPool } from './key-pool.ts'
import type { PoolDescriptor } from './key-pool.ts'
...
export interface AlternateKey extends ApiKeyPolicy {
  id: string
...
export interface ResolvedPiAiProviderProfile
  ...
  /** Immutable alternate-key descriptor; never contains credential values. */
  apiKeyPool?: PoolDescriptor
...
const apiKeyPolicy = z.object({
  enabled: z.boolean(),
  priority: z.number().step(1).min(0),
  weight: z.number().min(Number.MIN_VALUE),
})

const alternateKey = z.object({
  id: z.string().required(),
  credentialRef: z.string().required().role('credential-ref'),
  enabled: z.boolean(),
  priority: z.number().step(1).min(0),
  weight: z.number().min(Number.MIN_VALUE),
})

const apiKeyPool = z.object({
  mode: z.union(['priority', 'weighted']).default('priority'),
  primary: apiKeyPolicy,
  keys: z.array(alternateKey).default([]),
  maxAttempts: z.number().step(1).min(1),
  health: z.object({
    failureThreshold: z.number().step(1).min(1).default(3),
    openCircuitMs: z.number().step(1).min(1).max(MAX_TIMER_DELAY_MS).default(60_000),
  }),
})
