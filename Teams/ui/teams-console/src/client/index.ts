...
import { openCurrentAgentSession, openDshSession } from './dsh-adapter.ts'
...
export function apply(ctx: ClientContext): void {
  const controller = new TeamsConsoleController(
    (agent) => { openCurrentAgentSession(ctx.sessions, () => { controller.closeConsole() }, agent) },
    (sessionId) => { openDshSession(ctx.sessions, () => { controller.closeConsole() }, sessionId) },
  )
...
export { openCurrentAgentSession, openDshSession } from './dsh-adapter.ts'
