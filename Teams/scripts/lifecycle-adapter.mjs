...
function main() {
  ...
  const install = evidence(`${attempt}-install`, 'deployment_install', 'install', c, { artifact_hash: artifact.artifact_hash, environment_id: environmentId, entrypoint: installedUi, execution_surface: 'deployed_blackbox', producer: { adapter: 'project::deployment_adapter', identity: 'project::deployment_adapter' } })
  ...
  write(join(records, `pre-review-validation-record-${moduleId}.json`), {
    ...
    deployment: { environment_id: environmentId, install_receipt_id: install.evidence_id, restart_receipt_id: restart.evidence_id, entrypoint: installedUi, producer: { adapter: 'project::deployment_adapter', identity: 'project::deployment_adapter' }, observed_at: now() },
