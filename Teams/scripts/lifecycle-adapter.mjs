...
function main() {
  ...
  const whitebox = evidence(`${attempt}-whitebox`, 'development_whitebox', 'gate', c, { artifact_hash: artifact.artifact_hash, entrypoint: 'Teams UI/OpenCode test and build suite', execution_surface: 'development_whitebox' })
  ...
  run('pnpm', ['pack', '--pack-destination', deploymentRoot], join(root, 'ui', 'teams-console'))
  run('pnpm', ['pack', '--pack-destination', deploymentRoot], join(root, 'opencode-adapter'))
  ...
  run('npm', ['install', '--ignore-scripts', uiTarball, openCodeTarball], installRoot)
  const installedUi = join(installRoot, 'node_modules', '@deepseek-ai', 'teams-console', 'lib', 'client.js')
  const installedOpenCode = join(installRoot, 'node_modules', '@deepseek-ai', 'teams-opencode-adapter', 'lib', 'index.mjs')
  run('test', ['-s', installedUi], installRoot)
  run('test', ['-s', installedOpenCode], installRoot)
  const install = evidence(`${attempt}-install`, 'deployment_install', 'install', c, { artifact_hash: artifact.artifact_hash, environment_id: environmentId, entrypoint: installedUi, execution_surface: 'deployed_blackbox' })
  write(join(evidenceRoot, `${install.evidence_id}.json`), install)
  run('node', ['--check', installedOpenCode], installRoot)
  const restart = evidence(`${attempt}-restart`, 'deployment_restart', 'restart', c, { artifact_hash: artifact.artifact_hash, environment_id: environmentId, entrypoint: installedOpenCode, execution_surface: 'deployed_blackbox' })
  write(join(evidenceRoot, `${restart.evidence_id}.json`), restart)
  run('node', ['--input-type=module', '-e', `await import(${JSON.stringify(installedOpenCode)})`], installRoot)
  const blackbox = evidence(`${attempt}-blackbox`, 'deployed_blackbox', 'runtime', c, { artifact_hash: artifact.artifact_hash, environment_id: environmentId, entrypoint: installedUi, execution_surface: 'deployed_blackbox' })
  ...
  write(join(records, `pre-review-validation-record-${moduleId}.json`), {
    ...
    deployment: { environment_id: environmentId, install_receipt_id: install.evidence_id, restart_receipt_id: restart.evidence_id, entrypoint: installedUi, producer: { adapter, identity: `${adapter}/deployment` }, observed_at: now() },
