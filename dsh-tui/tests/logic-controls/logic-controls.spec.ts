...
function installAll(): { ctx: Context; registry: TuiLogicControlRegistry } {
  ...
  applyLogo(ctx)
  return { ctx, registry: ctx.tuiLogicControls }
}
...
test('projects input, status, connection, execution and logo without a renderer', () => {
  const { registry } = installAll()
  assert.deepEqual(registry.dispatch({ control: 'input', action: 'edit', text: 'hello', cursor: 5 }), {
    control: 'input', stableKey: 'control.input', text: 'hello', cursor: 5, mode: 'idle', revision: 1,
  })
  assert.deepEqual(registry.dispatch({ control: 'status', action: 'set', sessionId: 'session-1', cwd: '/tmp/dsh', mode: 'streaming' }), {
    control: 'status', stableKey: 'control.status', sessionId: 'session-1', cwd: '/tmp/dsh', mode: 'streaming', revision: 2,
  })
  assert.deepEqual(registry.dispatch({ control: 'connection', action: 'set', state: 'connected' }), {
    control: 'connection', stableKey: 'control.connection', state: 'connected', revision: 3,
  })
  assert.deepEqual(registry.dispatch({ control: 'execution', action: 'set', state: 'running', turnId: 'turn-1' }), {
    control: 'execution', stableKey: 'control.execution', state: 'running', turnId: 'turn-1', revision: 4,
  })
  assert.deepEqual(registry.dispatch({ control: 'logo', action: 'set', variant: 'compact', visible: false }), {
...
test('projection is consumable by a renderer seam without importing Ink or an agent', () => {
  ...
  const projection = registry.dispatch({ control: 'logo', action: 'set', variant: 'full', visible: true })
...
test('session selection is public-list scoped and slash commands are parse-only', () => {
  const { registry } = installAll()
  registry.dispatch({ control: 'session', action: 'snapshot', selectedSessionId: 'session-a', availableSessionIds: ['session-a', 'session-b'], cwd: '/tmp/dsh', lifecycle: 'active' })
  assert.deepEqual(registry.dispatch({ control: 'session', action: 'request-select', sessionId: 'session-b' }), {
  ...
  assert.deepEqual(registry.dispatch({ control: 'slash-command', action: 'project', input: '/resume session-b', command: '/resume', args: ['session-b'], accepted: true }), {
  ...
  assert.throws(() => registry.dispatch({ control: 'session', action: 'request-select', sessionId: 'session-b' }), /already active/)
  registry.dispatch({ control: 'session', action: 'snapshot', selectedSessionId: null, availableSessionIds: [], cwd: '/tmp/dsh', lifecycle: 'terminated' })
  assert.throws(() => registry.dispatch({ control: 'session', action: 'request-select', sessionId: 'session-b' }), /terminated session/)
  assert.throws(() => registry.dispatch({ control: 'session', action: 'snapshot', selectedSessionId: 'outside', availableSessionIds: [], cwd: '/tmp/dsh', lifecycle: 'active' }), /outside the owner-provided session scope/)
  const command = registry.dispatch({ control: 'slash-command', action: 'project', input: '/agent-private', command: '/agent-private', args: [], accepted: true }) as Extract<LogicControlProjection, { control: 'slash-command' }>
  assert.equal(command.accepted, true)
  assert.equal(command.command, '/agent-private')
  assert.throws(() => registry.dispatch({ control: 'slash-command', action: 'project', command: null, args: [], accepted: false }), /app-shell rejected slash command/)
  assert.throws(() => registry.dispatch({ control: 'slash-command', action: 'parse', input: '/' } as never), /unsupported logic control event/)
  assert.throws(() => registry.dispatch({ control: 'session', action: 'select', sessionId: 'session-b' } as never), /unsupported logic control event/)
})

test('rejects invalid transitions and control metadata leakage', () => {
  const { registry } = installAll()
  assert.throws(() => registry.dispatch({ control: 'input', action: 'submit', text: '' }), /empty input/)
  assert.deepEqual(registry.dispatch({ control: 'execution', action: 'set', state: 'failed', turnId: 'turn-1', message: 'owner reported failure' }), {
    control: 'execution', stableKey: 'control.execution', state: 'failed', turnId: 'turn-1', message: 'owner reported failure', revision: 1,
  })
  registry.dispatch({ control: 'connection', action: 'set', state: 'failed', message: 'offline' })
  assert.equal(registry.dispatch({ control: 'connection', action: 'set', state: 'connected' }).control, 'connection')
  assert.throws(() => registry.dispatch({ control: 'input', action: 'edit', text: 'x', cursor: 1, metadata: {} } as never), /control field 'metadata'/)
  assert.throws(() => registry.dispatch({ control: 'input', action: 'edit', text: 'x', cursor: 2 }), /outside text/)
  assert.throws(() => registry.dispatch({ control: 'connection', action: 'set', state: 'bogus' } as never), /connection.state is invalid/)
  assert.throws(() => registry.dispatch({ control: 'logo', action: 'set', variant: 'full', visible: true, extra: true } as never), /unknown control field 'extra'/)
...
test('rejects stale control events before state mutation', () => {
  const { registry } = installAll()
  registry.dispatch({ control: 'input', action: 'edit', text: 'new', cursor: 3, revision: 8 })
  assert.throws(() => registry.dispatch({ control: 'input', action: 'edit', text: 'old', cursor: 3, revision: 7 }), /newer than the global revision/)
  const projection = registry.project('input') as Extract<LogicControlProjection, { control: 'input' }>
  assert.equal(projection.text, 'new')
  assert.throws(() => registry.dispatch({ control: 'status', action: 'set', sessionId: null, cwd: null, mode: 'idle', revision: 8 }), /newer than the global revision/)
  assert.deepEqual(registry.error(), {
...
test('keeps error sequence monotonic after high-revision rejection and records public operation failures', () => {
  ...
  assert.throws(() => registry.dispatch({ control: 'input', action: 'edit', text: 'bad', cursor: 4, revision: 100 }), /outside text/)
  ...
  assert.deepEqual(registry.dispatch({ control: 'input', action: 'edit', text: 'ok', cursor: 2 }), {
...
test('closes unknown controls and malformed fields through the typed error chain', () => {
  const { registry } = installAll()
  assert.throws(() => registry.dispatch({ control: 'bogus', action: 'set' } as never), /unsupported logic control event/)
  assert.deepEqual(registry.error()?.control, 'unknown')
  assert.throws(() => registry.dispatch({ control: 'status', action: 'set', sessionId: 1, cwd: null, mode: 'idle' } as never), /status.sessionId must be a string/)
