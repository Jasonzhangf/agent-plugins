...
import { apply, componentRegistryServiceName, } from '../../playground/experiments/component-registry/src/component-registry.ts';
function install() {
    const ctx = new Context();
    apply(ctx);
    return ctx;
}
function nodeProps(value = { text: 'hello' }) {
    return {
...
function interactionProps(value = { focused: true }) {
    return {
...
function unsafeProps(value) {
    return value;
}
test('registers and resolves one exact effect-owned component', () => {
    const ctx = install();
    const render = (props) => textElement(props);
...
test('keeps control semantics outside the closed component envelope', () => {
    ...
    assert.throws(() => ctx.tuiComponentRegistry.render('conversation.cells', 'conversation.user', unsafeProps({ contract: 'tui.presentation-node.v1', node: { nodeId: 'x' } })), /invalid node/);
    assert.throws(() => ctx.tuiComponentRegistry.render('conversation.cells', 'conversation.user', unsafeProps({ contract: 'tui.presentation-node.v1', node: { nodeId: 'x', kind: 'conversation.user', publicationRevision: 1, lifecycle: 'settled', value: { control: true } }, control: true })), /unknown fields/);
});
...
test('rejects untyped props and untyped renderer output', () => {
    ...
    assert.throws(() => ctx.tuiComponentRegistry.render('conversation.cells', 'conversation.user', unsafeProps({ text: 'untyped' })), /typed TUI component contract/);
    assert.throws(() => ctx.tuiComponentRegistry.render('conversation.cells', 'conversation.user', nodeProps()), /typed TUI output contract/);
});
test('renders the closed interaction-state contract to terminal-neutral output', () => {
    ...
    assert.deepEqual(ctx.tuiComponentRegistry.render('bottom-pane.views', 'composer.editor', props), textElement(props));
});
