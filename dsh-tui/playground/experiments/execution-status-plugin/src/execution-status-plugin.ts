...
export class TuiExecutionStatusService extends Service implements TuiExecutionStatusFace {
  ...
  private disposed = false; private state: TuiExecutionState = 'idle'; private startedAt = 0; private title: string | null = null; private backgroundCount = 0; private revision = 0; private timer: ReturnType<typeof setInterval> | undefined; private readonly listeners = new Set<(projection: TuiExecutionStatusProjection) => void>()
  ...
  setBackgroundCount(count: number): void { if (this.disposed) throw new Error('execution-status-plugin: disposed'); if (!Number.isSafeInteger(count) || count < 0) throw new TypeError('execution-status-plugin: background count must be a non-negative integer'); if (this.backgroundCount === count) return; this.backgroundCount = count; this.revision += 1; this.publish() }
  ...
  project(now = Date.now()): TuiExecutionStatusProjection { const elapsedMs = this.state === 'idle' ? 0 : elapsed(this.startedAt, now); const background = this.backgroundCount > 0 ? ` · ${String(this.backgroundCount)} background · /jobs` : ''; const line = this.state === 'running' ? `${this.title ?? 'working'} ${activityFrame(elapsedMs)} ${formatElapsed(elapsedMs)} · Esc interrupt${background}` : null; return Object.freeze({ state: this.state, elapsedMs, title: this.title, backgroundCount: this.backgroundCount, line, revision: this.revision }) }
  ...
  dispose(): void { if (this.disposed) return; this.clearTimer(); this.disposed = true; this.state = 'idle'; this.title = null; this.backgroundCount = 0; this.listeners.clear() }
