...
if (!cmd) {
  console.error('[dsh-tui] pending-module requires a command name (typecheck|build:<m>|test:<m>)')
  process.exit(2)
}
...
if (cmd in testMap) {
  ...
  if (!existsSync(target)) {
    console.error(`[dsh-tui] missing test target: ${target}`)
    process.exit(2)
  }
...
console.error(`[dsh-tui] unknown command: ${cmd}`)
process.exit(2)
