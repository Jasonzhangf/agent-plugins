...
const cleanRoot = mkdtempSync(join(tmpdir(), 'dsh-tui-clean-install-'))
...
writeFileSync(join(installRoot, 'package.json'), `${JSON.stringify({
  name: 'dsh-tui-clean-install-verification',
  private: true,
  version: '0.0.0',
}, null, 2)}\n`)
...
if (lstatSync(installedPackagePath).isSymbolicLink()) fail('installed dsh-tui package is a symlink')
